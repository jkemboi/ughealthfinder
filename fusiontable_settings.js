...
$.extend(MapsLib, {
    ...
    searchPage: { 
        ...
        columns: [
            { label: "Facilitytype", type: "dropdown", foreach: "Facilitytype",
                entries: [
                    ["Any", "", true],
                ],
             },
    ...
    customInfoboxHtml: ' \
        ...
        <div class="entity orange_box {{row.Facilitytype}}"><span id="entity-text">.org</span></div> \
        <h4 class="infobox-header">{{row.Name}}</h4> \
        {{#if isListView}} \
            <p class="ui-li-desc infobox-subheader"> \
            {{row.Facilitytype}}<br> \
            {{row.Official Address}}</p> \
        {{else}} \
            <p></p><p class="ui-li-desc"> \
            {{row.Facilitytype}}<br> \
            {{row.Official Address}}<br> \
            {{#if row.coordinates}} \
                <aclass="btn" href="google.navigation:q={{row.coordinates}}" target="_blank"><i class="material-icons">&#xe569;</i></button></a><br> \
            {{/if}} \
            <a href="tel:1{{row.Official Mobile}}">{{row.Official Mobile}}</a></p> \
        {{/if}} \
        </p></div>',
